...
public class ItemSeed : Item, ITargetedAction<Ground> {
  public static int yieldCost = 20;
...
// #endif
  ...
  public IEnumerable<Ground> Targets(Player player) =>
    ...
    player.floor
      .soils
      .SelectMany(soil => soil.area)
      .Select(pos => player.floor.tiles[pos])
      .OfType<Ground>()
      .Where(ground => ItemPlaceableEntity.CanPlaceEntityOfType(plantType, ground));
